...
def prepare_match_model(root_dir):
  ...
  output_dir = os.path.join(res_dir, 'mlmatch', 
    'ek100_TBN_aug%s_VcTLate_thrPos%s_m%s.vis.%s%s.txt.%s.%d.loss.%s.af.%s.%d%s.glove.init.%dep'%
    (model_cfg.aug_chance, model_cfg.threshold_pos, model_cfg.margin, '-'.join(attn_ft_names),
