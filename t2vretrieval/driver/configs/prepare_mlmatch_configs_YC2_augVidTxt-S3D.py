...
def prepare_match_model(root_dir):
  ...
  attn_ft_names = ['S3D']
  num_words = len(np.load(os.path.join(anno_dir, 'int2word_youcook2.npy')))
  ...
  output_dir = os.path.join(res_dir, 'mlmatch', 
    'yc2_augVidTxt_pct%.2f_vis.%s%s.txt.%s.%d.loss.%s.af.%s.%d%s.glove.init.%sep'%
    (model_cfg.aug_chance, '-'.join(attn_ft_names),
  ...
  path_cfg = {
    ...
    'attn_ft_files': {},
    'name_file': {},
    'word2int_file': os.path.join(anno_dir, 'word2int_youcook2.json'),
    'int2word_file': os.path.join(anno_dir, 'int2word_youcook2.npy'),
    'dataset_file': {},
    'ref_caption_file': {},
    'ref_graph_file': {},
    'val_relevance_file': os.path.join(anno_dir, 'youcook2_rel_mat.pkl'),
    'relevance_file': os.path.join(anno_dir, 'youcook2_rel_mat.pkl'),
    'verb_classes': os.path.join(anno_dir, ''),
    'noun_classes': os.path.join(anno_dir, ''),
    'original_dataframe': os.path.join(anno_dir, 'YC2_retrieval_train.csv'),
    'video_noun_similars': os.path.join(anno_dir, 'YC2_video_verb_similars_coherent_nouns.json'),
    'video_verb_similars': os.path.join(anno_dir, 'YC2_video_verb_similars_coherent_verbs.json'),
  }
  for setname in ['trn', 'val', 'tst']:
    ...
    path_cfg['ref_caption_file'][setname] = os.path.join(anno_dir, 'ref_captions_youcook2.json')
    if setname == 'trn':
      path_cfg['dataset_file'][setname] = os.path.join(anno_dir, 'YC2_train.json')
