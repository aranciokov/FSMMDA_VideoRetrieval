...
def prepare_match_model(root_dir):
  ...
  output_dir = os.path.join(res_dir, 'mlmatch', 
    'ek100_TBN_aug%s_FixL_m%s.vis.%s%s.txt.%s.%d.loss.%s.af.%s.%d%s.glove.init.%dep'%
  ...
  path_cfg = {
    ...
    'video_verb_similars': os.path.join(anno_dir, "EK100_video_verb_similars.json"),
    'video_noun_similars': os.path.join(anno_dir, "EK100_video_noun_similars.json")
  }
